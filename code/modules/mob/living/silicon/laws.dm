...
/datum/law_manager
	...
	/// Blacklisted lawsets that cannot be viewed by admins nor players (as they do not work or make sense).
	var/static/list/datum/ai_laws/blacklisted_lawsets = list(
		/datum/ai_laws/custom,
		/datum/ai_laws/ratvar,
		/datum/ai_laws/pai,
		/datum/ai_laws/malfunction
	)
	/// All shown lawsets that can be selected.
	var/list/datum/ai_laws/selectable_laws

/datum/law_manager/New(mob/living/silicon/new_owner)
	..()
	owner = new_owner

	if(!selectable_laws)
		init_subtypes(/datum/ai_laws, selectable_laws)
		for(var/datum/ai_laws/lawset in selectable_laws)
			if(lawset in blacklisted_lawsets)
				selectable_laws -= lawset
...
/datum/law_manager/ui_data(mob/user)
	...
	data["lawsets"] = handle_lawsets(selectable_laws)
	return data

/datum/law_manager/proc/convert_to_bool(word)  // This proc only exists because changing the backend on lawstating is lots of effort.
	if(word == "Yes")
		return TRUE
	return FALSE

/// Sets the data based on the owner's lawset.
/datum/law_manager/proc/handle_laws(list/data)
	var/datum/ai_laws/laws = owner.laws
	
	var/list/devil[] = list()
	for(var/index = 1, index <= laws.devillaws.len, index++)
		devil[++devil.len] = list("law" = laws.devillaws[index], "index" = index, "indexdisplay" = 666,  "state" = convert_to_bool(owner.devillawcheck[index]), "type" = "devil", "hidebuttons" = data["isAntag"] ? TRUE : FALSE)
	data["devil"] = devil

	var/list/zeroth = list()
	if(laws.zeroth)
		zeroth[++zeroth.len] = list("law" = laws.zeroth, "index" = 0, "indexdisplay" = 0, "state" = convert_to_bool(owner.lawcheck[1]), "type" = "zeroth", "hidebuttons" = data["isAntag"] ? TRUE : FALSE)
	data["zeroth"] = zeroth

	var/list/hacked = list()
	for(var/index = 1, index <= laws.hacked.len, index++)
		hacked[++hacked.len] += list("law" = laws.hacked[index], "index" = index, "indexdisplay" = ionnum(), "state" = convert_to_bool(owner.hackedcheck[index]), "type" = "hacked", "hidebuttons" = FALSE)
	data["hacked"] = hacked

	var/list/ion = list()
	for(var/index = 1, index <= laws.ion.len, index++)
		ion[++ion.len] += list("law" = laws.ion[index], "index" = index, "indexdisplay" = ionnum(), "state" = convert_to_bool(owner.ioncheck[index]), "type" = "ion", "hidebuttons" = FALSE)
	data["ion"] = ion
...
/// Sets the radio channel data based on the owner's silicon type and their accessible radio channels.
...
/// Converts (a list of lawset datums) and returns a readable list of the lawsets for TGUI.
/datum/law_manager/proc/handle_lawsets(list/datum/ai_laws/laws)
	...
	for(var/datum/ai_laws/lawset in laws)
		...
		data[++data.len] = list("id" = lawset.id, "name" = lawset.name, "header" = lawset.law_header, "laws" = law_data)
